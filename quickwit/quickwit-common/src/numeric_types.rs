...
pub mod num_cmp {
    ...
    pub fn cmp_i64_f64(left_i: i64, right_f: f64) -> Result<Ordering, String> {
        if right_f.is_nan() {
            return Err("NaN comparison is not supported".to_string());
        }
    ...
    pub fn cmp_u64_f64(left_u: u64, right_f: f64) -> Result<Ordering, String> {
        if right_f.is_nan() {
            return Err("NaN comparison is not supported".to_string());
        }
